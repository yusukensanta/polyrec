...
- **Per-app audio tracks** — pick individual running apps with sound (Discord, Spotify, a game, etc.) as their own independent audio tracks, in an APPLICATIONS list alongside the audio devices. Each gets its own volume slider. Independent of App audio only / the video capture source — pick an app's audio here without it needing to be the window you're recording.
