...
- **Per-app audio tracks** (also in **🔊 Audio**) — pick individual running apps with sound (Discord, Spotify, a game, etc.) as their own independent audio tracks. Each gets its own volume slider. Independent of App audio only / the video capture source — pick an app's audio here without it needing to be the window you're recording. Two instances of the same app share one checkbox/slider. "+ Add app" pins an app (even one that isn't running yet) so it's remembered and auto-selected across launches.
